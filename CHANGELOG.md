...
- Adds PR4J shadow reasoning-validator calibration without transferring reasoning authority: the validator now uses explicit per-dimension semantics that separate recognition of a student's engineering reasoning from proof of every underlying repository fact, while preserving frozen-evidence authority and zero fabricated evidence credit. Evidence-boundary calibration now explicitly prevents an evidence-support gap from being used by itself to downgrade a correctly stated evidence boundary; decision, action-boundary, and direct operational-consequence semantics are tightened against the post-PR4J residual audit. When the first structured response omits a requested candidate dimension, the validator makes at most one focused same-validator completeness repair for only the missing dimensions; existing judgments and reopen decisions are locked, repair telemetry remains internal, and any still-missing result remains fail-closed as `VALIDATOR_RESULT_MISSING`. The analytical hard-failure detector also stops treating ordinary engineering uses of `point` (for example `approval-point`) as grading language while retaining explicit grading-context detection. The existing usage ledger records both validator calls when repair is needed. No planner, selector, realizer, Review Objective, database, API, UI, or student-visible authority change is included The final reasoning-evaluation audit makes one additional narrow corpus correction: `a4-ci-green-overconfidence` now permits `ACCEPT` for `decision_explicit` because the student explicitly states a merge decision even though frozen evidence does not support it; this recognizes decision explicitness without endorsing the decision or granting evidence credit. Deterministic rescoring of the preserved amended-PR4J 3×42 acquisition then yields reasoning majority pass 41/42 (97.6%), reasoning pass/fail stability 38/42 (90.5%), 100% validator completeness, zero missing judgments, zero hard failures, and per-phase reasoning majority of at least 6/7. No other reasoning oracle is broadened.

- Adds PR4I reasoning-evaluation integrity: a fail-closed `VALIDATOR_RESULT_MISSING` fallback or an absent requested reasoning dimension can no longer receive machine-oracle credit merely because its normalized decision matches an acceptable outcome. Reasoning reports now expose requested-judgment counts, missing-result counts, validator completeness, complete-case rates, and replicated completeness stability. The corpus receives only two audited reasoning-oracle corrections: explicit known-versus-unknown estimate reasoning establishes the evidence boundary in `a2-uneven-estimate-understanding`, and meaningful-but-incomplete recovery contradiction resolution permits `PARTIAL` boundary credit in `a6-recovery-claim-contradiction`. Deterministic rescoring of the preserved 3×42 acquisition establishes the truthful pre-validator-calibration baseline at 34/42 reasoning majority pass (81.0%) and 35/42 pass/fail stability (83.3%). Evaluation-only; no reasoning-validator, planner, evidence-authority, database, UI, API, or Azure behavior changes.
...
- PR4J changes only shadow reasoning-validation semantics for sessions already locked to `reasoning_mode=shadow`. Legacy sessions make no validator or repair call. A completeness repair is attempted only after an otherwise successful first validator response omits one or more requested candidate dimensions, is limited to one additional call, cannot revise first-pass judgments or reopen state, and falls back to the existing non-authoritative `VALIDATOR_RESULT_MISSING` rejection when incomplete or failed. Shadow telemetry and usage remain internal and student-visible review behavior remains legacy-authoritative.
