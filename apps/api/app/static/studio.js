...
function studentReviewReadiness(){
  ...
  if(!currentPhaseIsReleased())return {ready:false,code:'phase',title:`${currentPhase} has not been released yet`,detail:'Your instructor controls phase release for this section. You can review the phase question now, but a formal review cannot start until the phase is released.'};
  return {ready:true,code:'ready',title:'Review setup complete',detail:'GitHub identity, team repository, and current phase are ready.'};
}
function renderStudentReadiness(){
  ...
  const secondary=readiness.code==='github'
    ...
    : '';
  box.innerHTML=`<div><span class="eyebrow">${readiness.code==='phase'?'REVIEW NOT YET AVAILABLE':'SETUP REQUIRED'}</span><b>${escapeHtml(readiness.title)}</b><span>${escapeHtml(readiness.detail)}</span></div><div class="readiness-actions">${primary}${secondary}</div>`;
