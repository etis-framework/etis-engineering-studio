let sessionId=null,currentEvidence=null,currentPhase='A1',demoContext=null,currentView='studio',interactionMode='decision',currentChallenge=null,currentReviewer=null,committed=false,semanticReady=false,pending=false,courseModel=null,appRole='student',studentContext=null,selectedSectionId=null,reviewMode='board',requestedFindingId=null,selectedFindingIds=new Set(),healthState=null,authenticatedUser=null,engineeringEvidenceData=null,activeEvidenceLens=null,pendingEntryContext=null,composerContext=null,artifactContext=null,csrfToken=null;
...
const helpTopics={
 ...
 'decision-posture':{title:'What is a decision posture?',body:`<p>A posture is your current engineering recommendation, not a quiz answer. You can wait to select one until the conversation is clear.</p>`},
...
function studentReviewReadiness(){
  if(appRole!=='student'||!studentContext)return {ready:true,code:'ready',title:'',detail:''};
  const ob=studentContext.onboarding||{};
  if(!ob.team_assigned)return {ready:false,code:'team',title:'Team assignment required',detail:'Your instructor must assign you to a team before the Studio can use team repository evidence.'};
  if(!ob.github_identity)return {ready:false,code:'github',title:'Finish GitHub setup before your first review',detail:ob.repository_connected?'Connect your GitHub identity before starting a review.':'Connect your GitHub identity first. If the team repository is not connected yet, one teammate can connect and verify it next.'};
  if(!ob.repository_connected)return {ready:false,code:'repository',title:'Connect your team repository before starting a review',detail:'Only one teammate needs to connect and verify the repository. Once verified, the whole team inherits the connection.'};
  return {ready:true,code:'ready',title:'Review setup complete',detail:'GitHub identity and team repository are ready.'};
}
function renderStudentReadiness(){
  ...
  const secondary=readiness.code==='github'
    ...
    : '';
  box.innerHTML=`<div><span class="eyebrow">SETUP REQUIRED</span><b>${escapeHtml(readiness.title)}</b><span>${escapeHtml(readiness.detail)}</span></div><div class="readiness-actions">${primary}${secondary}</div>`;
...
function applyPhase(){currentPhase=phaseId();$('#gateQuestion').textContent=phaseQuestions[currentPhase]||'Can the team defend the current engineering gate?';$('#dimensionChips').innerHTML=(phaseDimensions[currentPhase]||[]).map(x=>`<span>${x}</span>`).join('')}
...
async function loadStudentContext(userId=null){const seed=userId?{user_id:userId}:await ensureDemo();studentContext=await fetch(`/api/v1/onboarding/users/${seed.user_id}`).then(r=>r.json());const sc=studentContext.sections?.[0];if(sc){selectedSectionId=sc.section.id;const team=sc.team;if(team){$('#contextTeam').textContent=team.name;$('#contextProject').textContent=team.project_name;els.repo.value=team.repo_full_name||'';$('#repoMode').innerHTML=`<span class="status ${sc.repository?'green':'amber'}"></span>${sc.repository?'Repository connected':'Repository setup needed'}`;}configurePhaseSelector(sc.phase_access)}renderMyTeam();renderStudentReadiness();updateStartReviewButton();setIdentity(currentView);return studentContext}
...
function updateStartReviewButton(){
  ...
  const readiness=studentReviewReadiness();
  if(appRole==='student'&&!readiness.ready){btn.textContent='Setup required before review';btn.disabled=true;btn.title=readiness.detail;return}
...
async function loadSemesterSetup(){try{const data=await adminSetupData();const sections=fillSectionSelectors(data),sel=$('#setupSectionSelector');selectedSectionId=Number(sel.value||sections[0]?.id||0);sel.onchange=loadSemesterSetup;const owner=!authenticatedUser||['course_owner','developer'].includes(authenticatedUser.role);['createTerm','createSection','archiveTerm'].forEach(id=>{const el=$('#'+id);if(el)el.classList.toggle('hidden',!owner)});const roleSelect=$('#staffRole');if(roleSelect){[...roleSelect.options].forEach(o=>o.disabled=!owner&&['course_owner','instructor'].includes(o.value));if(!owner&&['course_owner','instructor'].includes(roleSelect.value))roleSelect.value='ta'}if(!selectedSectionId){applySemesterLifecycleUi(null);return}const selected=sections.find(s=>Number(s.id)===selectedSectionId),term=selected?.term||null;await Promise.all([renderSetupTeams(),renderScheduleEditor(),renderStaff()]);applySemesterLifecycleUi(term);}catch(e){opsError('#scheduleEditor',safeErrorMessage(e),loadSemesterSetup)}}
