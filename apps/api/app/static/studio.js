...
async function loadStudentContext(userId=null){const seed=userId?{user_id:userId}:await ensureDemo();studentContext=await fetch(`/api/v1/onboarding/users/${seed.user_id}`).then(r=>r.json());const sc=studentContext.sections?.[0];if(sc){selectedSectionId=sc.section.id;const team=sc.team;if(team){$('#contextTeam').textContent=team.name;$('#contextProject').textContent=team.project_name;els.repo.value=team.repo_full_name||'';$('#repoMode').innerHTML=`<span class="status ${sc.repository?'green':'amber'}"></span>${sc.repository?'Repository connected':'Repository setup needed'}`;}configurePhaseSelector(sc.phase_access)}renderMyTeam();return studentContext}
...
function updateStartReviewButton(){
  ...
  if(!semanticReady){btn.textContent='Configure Coaching';btn.disabled=false;return}
